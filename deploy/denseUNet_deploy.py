...
def denseUNet_deploy():
    ...
    # Input data
    n.input_org = L.Input(input_param=dict(shape=dict(dim=[1, 3, 192, 192])))
    n.input_luma_org = L.Input(input_param=dict(shape=dict(dim=[1, 1, 192, 192])))
    n.shift_org = input_shifting_5x5(n.input_org)

    n.dum1 = L.DummyData(shape=dict(dim=[1, 3, 190, 190]))
    n.dum2 = L.DummyData(shape=dict(dim=[1, 3, 190, 190]))
    n.input_luma = L.Crop(n.input_luma_org, n.dum1, crop_param=dict(axis=2, offset=1))
    n.shift = L.Crop(n.shift_org, n.dum1, crop_param=dict(axis=2, offset=1))

    # Network
    n.conv1, n.poo1 = conv_conv_downsample_layer(n.input_luma, 3, 16, 2, 1)
    ...
    n.deconv9 = upsample_concat_crop_layer(n.conv9, n.conv1, 3, 64, 2, 0, 1)
...
if __name__ == "__main__":
    ...
    SRC_PATH = PATH + '/input_image.png'
    if not os.path.isfile(SRC_PATH):
        SRC_PATH = PATH + '/input_image.jpg'
        if not os.path.isfile(SRC_PATH):
    ...
    net.blobs['input_org'].data[...] = src_blob_color
    net.blobs['input_luma_org'].data[...] = src_blob_luma
    ...
    for i in range(25):
        ...
        cv2.imwrite(PATH+'/sai0_'+str(i)+'.png', dst)
    ...
    cv2.imwrite(PATH+'/result_lf.jpg', imgLF)
