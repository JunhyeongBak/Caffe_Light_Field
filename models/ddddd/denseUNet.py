...
def denseUNet(input, batch_size):
    def flow_layer(bottom=None, nout=1):
        flow_init = L.ReLU(bottom, in_place=True, engine=1)
        flow_init = L.Convolution(flow_init, num_output=nout, kernel_size=3, stride=1, dilation=1, pad=1,
                                group=1, bias_term=False, weight_filler=dict(type='msra'), bias_filler=dict(type='msra'), engine=1)
        return flow_init, flow_init
...
def denseUNet_train(script_path=None, data_path=None, tot=1, sai=25, shift_val=0.7, batch_size=1):
    ...
    n.loss2 = L.AbsLoss(n.depth_predict, n.depth_label)
    n.loss2 = L.Power(n.loss2, power=1.0, scale=0.5, shift=0.0, in_place=True, loss_weight=1)
...
if __name__ == "__main__":
    ...
    MODEL_PATH = './models/denseUNet_solver_iter_10000.caffemodel'
