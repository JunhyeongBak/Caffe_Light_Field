...
def shift_concat(shift_h, shift_v, sai):
    for i in range(sai):
        if i < sai-1:
            shift_h_slice, shift_h = L.Slice(shift_h, ntop=2, slice_param=dict(slice_dim=1, slice_point=[1]))
            shift_v_slice, shift_v = L.Slice(shift_v, ntop=2, slice_param=dict(slice_dim=1, slice_point=[1]))
            shift_hv_slice = L.Concat(*[shift_h_slice, shift_v_slice], concat_param={'axis': 1})
        else:
            shift_h_slice = shift_h
            shift_v_slice = shift_v
            shift_hv_slice = L.Concat(*[shift_h_slice, shift_v_slice], concat_param={'axis': 1})
        if i == 0:
            con = shift_hv_slice
        else:
            con = L.Concat(*[con, shift_hv_slice], concat_param={'axis': 1})
    return con
...
def denseUNet_train(script_path=None, data_path=None, tot=1, sai=25, shift_val=0.7, batch_size=1):
    ...
    f = open(data_path+'_depth'+'/train_depth_source'+str(12)+'.txt', 'w')
    for i_tot in range(tot):
        data = data_path+'_depth'+'/sai'+str(i_tot)+'_'+str(40)+'.png 0'+'\n'
        f.write(data)
    f.close()
    ...
    # Data loading
    n.input, n.trash = image_data_center(batch_size=batch_size, data_path=data_path, source='train_source')
    n.depth, n.trash = image_data_center(batch_size=batch_size, data_path=data_path+"_depth", source='train_depth_source')
    n.shift_depth_h = input_shifting(center=n.depth, batch_size=batch_size, sai=sai, shift_val=shift_val)
    n.shift_depth_v = input_shifting(center=n.depth, batch_size=batch_size, sai=sai, shift_val=shift_val)
    n.shift_depth = shift_concat(n.shift_depth_h, n.shift_depth_v, sai)

    # Fundamental Network
    n.flow = denseUNet(n.input, batch_size)   
    #n.flow2 = L.Convolution(n.shift_depth, num_output=50, kernel_size=1, stride=1, dilation=1, pad=0,
    #                            group=1, bias_term=True, weight_filler=dict(type='msra'), bias_filler=dict(type='msra'), engine=1)
    n.flow2 = L.Convolution(n.shift_depth, num_output=50, kernel_size=1, stride=1, dilation=1, pad=0,
                                group=50, bias_term=False, weight_filler=dict(type='msra'), bias_filler=dict(type='msra'), engine=1)
    ...
    n.flow_h2, n.flow_v2 = L.Slice(n.flow2, ntop=2, slice_param=dict(slice_dim=1, slice_point=[25]))
    n.predict2 = slice_warp2(n.shift, n.flow_h2, n.flow_v2, sai)

    # Loss
    n.label, n.trash_tot = image_data(batch_size=batch_size, data_path=data_path, source='train_source', sai=sai)
    ...
    n.loss = L.AbsLoss(n.predict, n.label)
    n.loss = L.Power(n.loss, power=1.0, scale=1.0, shift=0.0, in_place=True, loss_weight=1)
    n.loss2 = L.AbsLoss(n.predict2, n.label)
    n.loss2 = L.Power(n.loss2, power=1.0, scale=1.0, shift=0.0, in_place=True, loss_weight=1)
    n.loss3 = L.AbsLoss(n.flow, n.flow2)
    n.loss3 = L.Power(n.loss3, power=1.0, scale=0.1, shift=0.0, in_place=True, loss_weight=1)
    #n.silence = L.Silence(n.depth, ntop=0)
...
def denseUNet_test(script_path=None, data_path=None, tot=1, sai=25, shift_val=0.7, batch_size=1):
    ...
    f = open(data_path+'_depth'+'/test_depth_source'+str(12)+'.txt', 'w')
    for i_tot in range(tot):
        data = data_path+'_depth'+'/sai'+str(i_tot)+'_'+str(40)+'.png 0'+'\n'
        f.write(data)
    f.close()
    ...
    # Data Loading
    n.input, n.trash = image_data_center(batch_size=batch_size, data_path=data_path, source='test_source')
    n.depth, n.trash = image_data_center(batch_size=batch_size, data_path=data_path+"_depth", source='test_depth_source')
    n.shift_depth_h = input_shifting(center=n.depth, batch_size=batch_size, sai=sai, shift_val=shift_val)
    n.shift_depth_v = input_shifting(center=n.depth, batch_size=batch_size, sai=sai, shift_val=shift_val)
    n.shift_depth = shift_concat(n.shift_depth_h, n.shift_depth_v, sai)
    ...
    n.flow_h2, n.flow_v2 = L.Slice(n.flow2, ntop=2, slice_param=dict(slice_dim=1, slice_point=[25]))
    n.flow_con2 = L.Concat(*[n.flow_v2, n.flow_h2], concat_param={'axis': 1})
    ...
    param_str = json.dumps({'flow_size': 25, 'color_size': 1})
    n.predict2 = L.Python(*[n.shift, n.flow_con2], module = 'bilinear_sampler_layer_3ch', layer = 'BilinearSamplerLayer3ch', ntop = 1, param_str = param_str)

    # Visualization
    n.label, n.trash_tot = image_data(batch_size=batch_size, data_path=data_path, source='test_source', sai=sai)
    ...
    n.trash6 = L.Python(n.predict2, module='visualization_layer', layer='VisualizationLayer', ntop=1,
                    param_str=str(dict(path='./datas/face_dataset', name='predict2', mult=1*256)))
    n.trash7 = L.Python(n.flow_h2, module='visualization_layer', layer='VisualizationLayer', ntop=1,
                    param_str=str(dict(path='./datas/face_dataset', name='flow_h2', mult=20)))
    n.trash8 = L.Python(n.flow_v2, module='visualization_layer', layer='VisualizationLayer', ntop=1,
                    param_str=str(dict(path='./datas/face_dataset', name='flow_v2', mult=20)))
...
def denseUNet_deploy(script_path=None, batch_size=1, sai=25, shift_val=0.7):
    ...
    # Network
    n.flow = denseUNet(n.input_luma_down, batch_size)  
...
def denseUNet_runner(script_path=None, model_path=None, src_color=None, n=None):
    ...
    flow = np.zeros((192, 192, 2))
    flow_color_list = np.zeros((192, 192, 3, 5*5))
...
def denseUNet_tester(script_path=None,
    ...
    for i_tot in range(test_range):
        ...
        src_img = cv2.resize(src_img, (192, 192), interpolation=cv2.INTER_AREA)
...
if __name__ == "__main__":
    ...
    MODEL_PATH = './models/denseUNet_solver_iter_100000.caffemodel'
