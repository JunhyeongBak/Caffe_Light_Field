...
def denseUNet(input, batch_size):
    def flow_layer(bottom=None, nout=1):
        conv = L.Convolution(bottom, num_output=1, kernel_size=1, stride=1, dilation=1, pad=0,
                                group=1, bias_term=False, weight_filler=dict(type='msra'), bias_filler=dict(type='msra'), engine=1)
        flow = L.ReLU(conv, relu_param=dict(negative_slope=0.2), in_place=True, engine=1)
        flow = L.Convolution(flow, num_output=nout, kernel_size=3, stride=1, dilation=1, pad=1,
                                group=1, bias_term=False, weight_filler=dict(type='msra'), bias_filler=dict(type='msra'), engine=1)
        flow = L.ReLU(flow, relu_param=dict(negative_slope=0.2), in_place=True, engine=1)
        flow = L.Convolution(flow, num_output=nout, kernel_size=5, stride=1, dilation=1, pad=2,
                                group=nout, bias_term=False, weight_filler=dict(type='msra'), bias_filler=dict(type='msra'), engine=1, param=[dict(lr_mult=0.5, decay_mult=0)])
        return flow, conv
    ...
    deconv1 = upsample_concat_layer(feature, conv4, 128, 32)
    deconv1 = conv_conv_group_layer(deconv1, 128)
    deconv2 = upsample_concat_layer(deconv1, conv3, 64, 64)
    deconv2 = conv_conv_group_layer(deconv2, 64)
    deconv3 = upsample_concat_layer(deconv2, conv2, 64, 128)
    deconv3 = conv_conv_group_layer(deconv3, 64)
    deconv4 = upsample_concat_layer(deconv3, conv1, 64, 256)
...
def denseUNet_train(script_path=None, data_path=None, tot=1, sai=25, shift_val=0.7, batch_size=1):
    ...
    # Fundamental Network
    n.flow, n.conv = denseUNet(n.input, batch_size)
    ...
    n.loss2 = L.AbsLoss(n.conv, n.depth)
...
def denseUNet_deploy(script_path=None, batch_size=1, sai=25, shift_val=0.7):
    ...
    # Network
    n.flow, n.conv = denseUNet(n.input_luma_down, batch_size)  
...
def denseUNet_solver(script_train_path=None, script_test_path=None, solver_path=None, snapshot_path=None):
    ...
    s.max_iter = 500000
    s.type = 'Adam'
    s.base_lr = 0.005 # 0.0005,  0.0000001 # 0.000005(basic), 0.0000001
    s.lr_policy = 'step'
    s.gamma = 0.75
    s.power = 0.75
    s.stepsize = 1000
...
if __name__ == "__main__":
    ...
    MODEL_PATH = './models/denseUNet_solver_iter_60000.caffemodel'
