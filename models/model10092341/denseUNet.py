...
def image_data_center(batch_size=1, data_path=None, source='train_source', center_id=27):
...
def image_data(batch_size=1, data_path=None, source='train_source', grid_size=64):
...
def denseUNet_train(script_path=None, data_path=None, tot=1, sai=25, batch_size=1):
    ...
    for i_sai in range(sai):
        ...
        for i_tot in range(tot):
            data = data_path+'/sai'+str(i_tot)+'_'+str(i_sai)+'.png 0'+'\n'
            f.write(data)
        f.close()
    ...
    # Data loading
    n.input, n.trash = image_data_center(batch_size, data_path, 'train_source', 40)
...
def denseUNet_test(script_path=None, data_path=None, tot=1, sai=25, batch_size=1):
    ...
    # Data Loading
    n.input, n.trash = image_data_center(batch_size, data_path, 'val_source', 40)
...
if __name__ == "__main__":
    ...
    #solver.net.copy_from(MODEL_PATH+'/denseUNet_solver_iter_44659.caffemodel')
    solver.solve()

    #test_img = cv2.imread('./test.png', 1)
    #test_list = denseUNet_runner(script_path=DEPLOY_PATH, model_path=MODEL_PATH+'/denseUNet_solver_iter_44659.caffemodel', src_color=test_img)
